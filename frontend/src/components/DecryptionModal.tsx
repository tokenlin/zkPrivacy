...
import { getkH_Hash } from "./contract/zkPrivacy";
...
export function DecryptionModal({
  ...
  useEffect(() => {
    console.log("proofArgsStr: ", proofArgsStr);
  ...
  const sendRequest = async () => {
    ...
    console.log("sendRequest-proofArgs: ", proofArgs);
    console.log("sendRequest-proof: ", proof);
    const args = parseKS(proof);  // from input value directly
  ...
  const handleSubmit = async () => {
    console.log("proof.trim(): ", proof.trim());
  ...
  const checkTransaction = async () => {
    ...
    const intervalId = setTimeout(async () => {
      ...
      if (s_lastRequestId !== previousRequestId) {
        let hash = "";
        if (s_lastResponse){
          hash = await getkH_Hash(s_lastResponse);
          hash = "depositor hash: " + hash;
        }
        setStatus("success");
        setResult({
          id: s_lastRequestId,
          // result: s_lastResponse || s_lastError,
          result: hash || s_lastError,
        });
