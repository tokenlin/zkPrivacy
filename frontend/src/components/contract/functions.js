...
export function parseKS (kS_str) {
  let kS;
  if (typeof(kS_str) == "string"){
    kS_str = kS_str.replaceAll("0x", "");
    let temp = ["0x"+kS_str.slice(0,64), "0x"+kS_str.slice(64,128)];
    kS = [BigInt(temp[0]), BigInt(temp[1])]
  }else{
    kS = [BigInt(kS_str[0]), BigInt(kS_str[1])]
  }

  console.log("parseKS-kS: ", kS);
