...
export async function deposit () {
  ...
  const note = `zkPrivacy-eth-0.001-31337-${toHex(deposit.preimage, 31)}`;
  ...
  // const note = `zkPrivacy-eth-1-1-${toHex(deposit.preimage, 62)}`;
...
async function parseNote (noteString) {
  const noteRegex =
    /zkPrivacy-eth-0.001-(?<chainId>\d+)-0x(?<note>[0-9a-fA-F]{62})/g;
...
// kH_Str = 0x10c926fc456a4aa538d81de48d5ccad5e4f657a14c8e7523a6c48e2efede547a14f5fcbc32ce835b28febedb0bd6788e9b02d18fd3459c6b203c17486c89d4c5
// kH_Hash:  0x0ec76ab52a26090aab1990f1008e38dcebf7c0520b5147e13cd398074e09c694
export async function getkH_Hash(kH_Str){

  kH_Str = kH_Str.replaceAll("0x", "");

  let publicKey_str = ["0x" + kH_Str.slice(0, 64), "0x" + kH_Str.slice(64, 128)];
  let publicKey = [BigInt(publicKey_str[0]), BigInt(publicKey_str[1])];
  console.log("publicKey_str: ", publicKey_str);
  console.log("publicKey: ", publicKey);
  ...
  const hash = await perdersenHash(uint8Array);

  console.log("getkH_Hash from kH_Str: ", toHex(hash, 32))
